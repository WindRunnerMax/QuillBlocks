...
describe("collect pick", () => {
  it("pick op", () => {
    ...
    const op = editor.collect.pickOpAtPoint(new Point(0, 1));
  ...
  it("pick op attributes", () => {
    const delta = new Delta().insert("text").insert("text", { bold: "true" }).insert("\n");
    const editor = new Editor({ delta });
    const op = editor.collect.pickOpAtPoint(new Point(0, 5));
  ...
  it("pick op end", () => {
    const delta = new Delta().insert("text").insert("text", { bold: "true" }).insert("\n");
    const editor = new Editor({ delta });
    const op = editor.collect.pickOpAtPoint(new Point(0, 4));
    expect(op).toEqual({ insert: "text" });
  });
  ...
  it("pick op overflow", () => {
    const delta = new Delta().insert("text").insert("text", { bold: "true" }).insert("\n");
    const editor = new Editor({ delta });
    const op = editor.collect.pickOpAtPoint(new Point(0, 10));
